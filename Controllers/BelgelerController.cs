...
namespace KcetasWeb.Controllers
{
    ...
    public class BelgelerController : Controller
    {
        ...
        public async Task<IActionResult> Index(KcetasWeb.ViewModels.BelgelerListeViewModel filtre)
        {
            ...
            viewModel.PageSize = viewModel.PageSize > 0 && viewModel.PageSize != 50 ? viewModel.PageSize : 50;
            ...
            {
                // API'den Server-Side Pagination ile çekiyoruz (Belleği Yormamak İçin)
                var pagedFatura = await _faturaService.GetPagedAsync(viewModel.CurrentPage, viewModel.PageSize, viewModel.FiltreBelgeNo, null, null);
                var faturalar = pagedFatura.Data ?? new List<Fatura>();
                totalItems = pagedFatura.TotalCount;

                var sozlesmelerD = (await _sozlesmeService.GetAllAsync()).GroupBy(s => s.sozlesme_id).ToDictionary(g => g.Key, g => g.First());
                ...
                foreach (var f in faturalar)
                {
                    ...
                    // Client-side filtering for unsupported API filters on current page
                    if (!string.IsNullOrEmpty(viewModel.FiltreTekilKod) && 
                        !gercekTekilKod.Contains(viewModel.FiltreTekilKod, StringComparison.OrdinalIgnoreCase))
                        continue;
                        
                    var fTarih = f.fatura_tarihi.HasValue ? f.fatura_tarihi.Value.ToDateTime(TimeOnly.MinValue) : (f.created_at ?? DateTime.Now);
                    if (viewModel.FiltreBelgeTarihi.HasValue && fTarih.Date != viewModel.FiltreBelgeTarihi.Value.Date)
                        continue;
            ...
            else if (viewModel.AktifSekme == "Tutanak")
            {
                ...
                var isEmirleri = tumIsEmirleri.Where(ie => ie.durum == KcetasWeb.Models.Enums.IsEmriDurumu.Tamamlandi).ToList();
                
                foreach (var ie in isEmirleri)
                {
                    var tn = tnMap.ContainsKey(ie.tuketim_noktasi_id) ? tnMap[ie.tuketim_noktasi_id] : null;
                    var tekilKod = tn != null ? tn.tekil_kod : $"TK-ID-{ie.tuketim_noktasi_id}";
                    
                    if (!string.IsNullOrEmpty(viewModel.FiltreTekilKod) && !tekilKod.Contains(viewModel.FiltreTekilKod, StringComparison.OrdinalIgnoreCase))
                        continue;
                        
                    var tutanakNo = !string.IsNullOrEmpty(ie.tutanak_no) ? ie.tutanak_no : $"TUT-{ie.is_emri_id}";
                    if (!string.IsNullOrEmpty(viewModel.FiltreBelgeNo) && !tutanakNo.Contains(viewModel.FiltreBelgeNo, StringComparison.OrdinalIgnoreCase))
                        continue;
                        
                    var tTarih = ie.updated_at ?? ie.created_at;
                    if (viewModel.FiltreBelgeTarihi.HasValue && tTarih.Date != viewModel.FiltreBelgeTarihi.Value.Date)
                        continue;
                ...
                totalItems = tumBelgeler.Count;
                tumBelgeler = tumBelgeler.Skip((viewModel.CurrentPage - 1) * viewModel.PageSize).Take(viewModel.PageSize).ToList();
            }
            else if (viewModel.AktifSekme == "Sozlesme")
            {
                ...
                foreach (var s in sozlesmeler)
                {
                    ...
                    var tekilKod = tn != null ? tn.tekil_kod : $"TK-ID-{s.tuketim_noktasi_id}";
                    
                    if (!string.IsNullOrEmpty(viewModel.FiltreTekilKod) && !tekilKod.Contains(viewModel.FiltreTekilKod, StringComparison.OrdinalIgnoreCase))
                        continue;
                        
                    if (!string.IsNullOrEmpty(viewModel.FiltreBelgeNo) && !s.sozlesme_no.Contains(viewModel.FiltreBelgeNo, StringComparison.OrdinalIgnoreCase))
                        continue;
                        
                    if (viewModel.FiltreBelgeTarihi.HasValue && s.created_at.Date != viewModel.FiltreBelgeTarihi.Value.Date)
                        continue;
            ...
            viewModel.Belgeler = tumBelgeler;
